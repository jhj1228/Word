...
static bool beginPlay = true; // This var is only true at the beginning of the game, used for the first matrix creations
...
static bool CreatePiece()
{
    piecePositionX = static_cast<int>((GRID_HORIZONTAL_SIZE - 4) / 2);
    piecePositionY = 0;
...
static void CheckDetection(bool* detection)
{
    for (int j = GRID_VERTICAL_SIZE - 2; j >= 0; j--)
    {
        for (int i = 1; i < GRID_HORIZONTAL_SIZE - 1; i++)
        {
            if ((grid[i][j] == GridSquare::MOVING) && ((grid[i][j + 1] == GridSquare::FULL) || (grid[i][j + 1] == GridSquare::BLOCK)))
                *detection = true;
        }
    }
}
